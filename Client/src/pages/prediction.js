...
export function predictionPage(container) {
  ...
  main.innerHTML = `
    <header class="page-header">
      <h1><i class="fa-solid fa-wand-magic-sparkles" style="color: var(--fire-orange); margin-right: 8px;"></i> Prédiction des incendies</h1>
    </header>
    <div class="page-body">
      <div class="prediction-layout">
        <div class="prediction-form" style="display:flex; flex-direction:column; gap:20px;">
          <div>
            <h3 style="margin-top:0;"><i class="fa-solid fa-clipboard-list" style="color: var(--fire-orange); margin-right: 6px;"></i> Données de prédiction</h3>
          ...
          <form id="predict-form">
            ...
            <div id="weather-preview" style="display:none; background:rgba(255,255,255,0.02); border: 1px solid var(--border-color); padding:16px; border-radius:var(--radius-sm); margin-bottom:20px;">
              <div id="weather-preview-title" style="font-size:11px; font-weight:700; color:var(--fire-orange); letter-spacing:1px; text-transform:uppercase; margin-bottom:10px;"><i class="fa-solid fa-cloud-sun" style="color: var(--fire-orange); margin-right: 6px;"></i> Météo de la Province</div>
            ...
            <button type="submit" class="btn-primary" style="width:100%; padding:12px; font-weight:600; cursor:pointer; background:var(--fire-gradient); border:none; border-radius:var(--radius-sm); color:white; display:flex; justify-content:center; align-items:center; gap:8px;">
              <span><i class="fa-solid fa-brain" style="margin-right: 6px;"></i> Lancer la Prédiction IA</span>
            </button>
          </form>
        </div>
        ...
        <div class="prediction-result" id="pred-result" style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 28px; display: flex; flex-direction: column; min-height: 400px; justify-content: center; align-items: center; text-align: center;">
          <div class="empty-state">
            <div class="icon" style="font-size: 48px; margin-bottom: 16px; color: var(--fire-orange);"><i class="fa-solid fa-wand-magic-sparkles"></i></div>
  ...
  document.getElementById('predict-form').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      resultEl.innerHTML = `
        <div class="empty-state">
          <div class="icon" style="font-size:40px; margin-bottom:12px; color: var(--fire-red);"><i class="fa-solid fa-circle-xmark"></i></div>
  ...
  try {
    ...
    if (date) {
      ...
      if (date === todayStr) {
        ...
        if (title) title.innerHTML = "<i class='fa-solid fa-sun' style='color: var(--fire-orange); margin-right: 6px;'></i> Météo Actuelle de la Province";
      } else {
        ...
        if (title) {
          ...
          title.innerHTML = `<i class="fa-solid fa-calendar-days" style="color: var(--fire-orange); margin-right: 6px;"></i> Météo Archivée (${dateFormatted})`;
        }
      }
    } else {
      w = await api.getWeather();
    ...
    if (w && w.temperature !== null && w.temperature !== undefined) {
      const windDir = w.wind_direction ? ` &nbsp; <i class="fa-solid fa-compass" style="color:var(--fire-orange)"></i> Vent: ${w.wind_direction}` : '';
      const humidityStr = w.humidity !== undefined ? ` &nbsp; <i class="fa-solid fa-droplet" style="color:var(--risk-low)"></i> Humidité: ${w.humidity}%` : '';
      data.innerHTML = `<span><i class="fa-solid fa-temperature-three-quarters" style="color:var(--fire-orange)"></i> Temp: <strong>${w.temperature}°C</strong></span>${humidityStr} <span><i class="fa-solid fa-wind" style="color:var(--text-secondary)"></i> Vent: <strong>${w.wind_speed} km/h</strong></span>${windDir}`;
    ...
    try {
      ...
      data.innerHTML = `<i class="fa-solid fa-temperature-three-quarters" style="color:var(--fire-orange)"></i> ${w.temperature}°C &nbsp; <i class="fa-solid fa-droplet" style="color:var(--risk-low)"></i> ${w.humidity}% &nbsp; <i class="fa-solid fa-wind" style="color:var(--text-secondary)"></i> ${w.wind_speed} km/h (Actuel)`;
      if (title) title.innerHTML = "<i class='fa-solid fa-sun' style='color: var(--fire-orange); margin-right: 6px;'></i> Météo (Données Actuelles de Fallback)";
      el.style.display = 'block';
    } catch(err) {}
  }
}
  ...
  container.innerHTML = `
    <div style="width:100%; text-align:center;">
      ...
      <p style="margin-top:20px; font-size:12.5px; color:var(--text-secondary); text-align:center; padding: 12px; background: rgba(255,255,255,0.02); border-radius: var(--radius-sm); border: 1px solid var(--border-color); line-height:1.5;">
        ${risk >= 0.7 ? '<i class="fa-solid fa-triangle-exclamation" style="color: var(--fire-red);"></i> <strong>Alerte Critique :</strong> Risque extrême d\'embrasement détecté. Déploiement préventif des équipes recommandé.' : risk >= 0.4 ? '<i class="fa-solid fa-triangle-exclamation" style="color: var(--fire-yellow);"></i> <strong>Vigilance requise :</strong> Indice moyen. Surveiller activement les conditions de sécheresse et de vent.' : '<i class="fa-solid fa-circle-check" style="color: var(--risk-low);"></i> <strong>Conditions normales :</strong> Indice faible. Le massif forestier ne présente pas de danger imminent actuellement.'}
      </p>
    </div>
  `;
}
